...
class VisibilityHTMLPlots:
    ...
    def process_visibilities(self, visibility_files):
        ...
        html = open(f"{self.control['output-path']}{self.control['observation']}/{self.scan}/index.html", "w")
        ...
        for i, freq in enumerate(self.freq_block):
            ...
            if current_polarization in [0, 3]:
                for baseline in [0, 2]:
                    image_directory = f"{self.control['output-path']}{self.control['observation']}/{self.scan}/out/plots/auto_correlations/"
                    ...
                    html.write(
                        f"<img src=\"./out/plots/auto_correlations/{filename}{baselines[baseline]}_{current_polarization}.png\" alt=\"Image\">")
                    html.write("</th>")
            else:
            ...
            self.save_plot(x, amplitude, f"Amplitude {baselines[1]}",
                      ...
                      f"{self.control['output-path']}{self.control['observation']}/{self.scan}/out/plots/cross_correlations/amplitude/")

            # Phase Plot
            self.save_plot(x, phase, f"Phase {baselines[1]}",
                      "Frequency (MHz)", "Phase", f"{filename}{baselines[1]}_phase_{current_polarization}.png",
                      f"{self.control['output-path']}{self.control['observation']}/{self.scan}/out/plots/cross_correlations/phase/")
            ...
            self.save_plot(lag_x, lag_y, f"Lag {baselines[1]}",
                      ...
                      f"{self.control['output-path']}{self.control['observation']}/{self.scan}/out/plots/cross_correlations/lag/")

            image_directory = f"./out/plots/cross_correlations/lag/"
            ...
            image_directory = f"./out/plots/cross_correlations/amplitude/"
            html.write("<th class=\"image-link\">")
            html.write(
            ...
            image_directory = f"./out/plots/cross_correlations/phase/"
            html.write("<th class=\"image-link\">")
            html.write(
