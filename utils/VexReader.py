...
class VexReader:
    ...
    def sample_rate(self):
        ...
        sample_rate = str(
            int(float(self.vex_file['FREQ'][freq]['sample_rate'].split()[0]) * 1e+6))
    ...
    def generate_threads_block(self):
        bitstreams, threads, modes = create_bitstreams_and_threads_block(
            self.vex_path)
    ...
    def center_frequencies(self):
        ...
        for i in range(0, len(self.vex_file['FREQ'][dict_key].getall('chan_def')), 2):
            center_frequencies.append(
                float(self.vex_file['FREQ'][dict_key].getall('chan_def')[i][1].split()[0]))
    ...
    def get_scans(self):
        return list(self.vex_file['SCHED'].keys())

    def get_observation(self):
        return self.vex_file['GLOBAL']['EXPER']
